...
% Procurar no poner superlativos como 'the most'a menos que tengamos
% los datos - M
% Anyway, you should state the problem and context first, not what you
% are going to do. Also, valuable for what kind of problems? - JJ
...
% You should say something about topologies and their influence on the
% final result. Why chain? Why hypercube? If topology is the focus,
% different options should be checked - JJ
...
% The state of the art refers to different methods *solving the same
% problem* and will be used to compare against. You need to qualify
% the problem domain - JJ

%\subsection{Swarm Intelligence}

%“Swarm intelligence (SI) is the collective behavior of decentralized,
%self-organized, natural or artificial systems.” The expression was
%first brought up by Gerardo Beni and Jing Wang in 1989 in the context
%of robotic cellular systems \cite{b4}.

%SI systems are mostly made of a population of simple particles that
%interact with each other and with their environment at a local
%level. The inspiration to make this kind of system comes from nature,
%especially biological systems. Particles follow very simple rules for
%their interactions and these interactions lead to the creation of a
%global "smart" behavior \cite{b5}.
% These paragraphs above could be suppressed - JJ

%\subsection{PSO}
% Agregar un párrafo que conecte con esto:

%Knowing that it is planned to work with different populations of swarms, the
%need to use an optimization algorithm that has an affinity with this technique
%and with swarm intelligence is generated. From what has been observed in
%countless works, some of them mentioned in this paper, PSO has great
%compatibility with the techniques that are intended to be used.

% The remaining text is basically taken from Wikipedia and other
% article. This needs to be rewritten basically from scratch.
...
% The state of the art should be an exposition of what is the current
% *state* of the problem that will be solved. - JJ

%\subsection{Multi-Swarm PSO}

% Still no idea of the problem that is going to be solved, and why we
% need multiple swarms. Also, it should go to the state of the art if
% this is the focus of the paper - JJ
...
new regions promising to "keep going." % This should probably go to
                                % state of the art - JJ

In contrast to typical PSO swarms, sub-swarms are fed with 
information % you should qualify "some" - jj
about previous swarms. These could be the positions and velocities
instead of having their initial parameters to be randomly selected. In
general, the development of multi-swarm systems creates a new path of
design decisions that did not exist during the original development of
PSO. These design decisions have been have well-established guidelines
thanks to the numerous studies on the topic, for example, the use of
non-random initial positions and initial velocities leads to better
re-sults in multi-swarm systems, which is not the case for individual
swarms \cite{b9}. 
...
\begin{itemize}
    ...
    \item A connection topology that defines how to connect sub-populations.
      ...
      % This should go to the introduction. - JJ
...
that simple share the global best of all the swarms\cite{b16}. % State
                                % of the art should be the best place
                                % for this - JJ
...
were used to improve the parallel optimization\cite{b17}. % Definitely
                                % state of the art - JJ
...
\cite{b18}. % Better a higher-level description, describing what's in
            % the queues. Also, message queues. - JJ
This structure  allows the user to work on several populations and
allow different optimization algorithms % (See Figure \ref{fig:example}).
...
\subsection{EvoSwarm communication topology} % These sections are too
...
the % Are you sure about capitalizing hypercube? - JJ
...
with an $\alpha$ of 0.5, giving a critical value to exceed 1.64; % meaning? - JJ
