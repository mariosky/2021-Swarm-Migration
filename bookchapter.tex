...
MSE as reference. % Probably rewrite it. Why did you do it? Why did
...
(PSO) based on multiple swarms (or sub-swarm) instead of one. The basic flow in a multi-swarm optimization is that each sub-swarm focuses on a specific search space region. In contrast, a specific diversification method decides where and when to launch the sub-swarms. 
...
In contrast to typical PSO swarms, sub-swarms are fed with some
